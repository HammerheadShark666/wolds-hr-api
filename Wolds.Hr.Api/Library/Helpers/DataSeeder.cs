...
internal class DataSeeder(WoldsHrDbContext woldsHrDbContext) : IDataSeeder
{
    public async Task SeedDatabaseAsync()
    {
        ...
        if (departments != null)
            await SeedEmployeesAsync(departments);
    }

    private async Task SeedAccountAsync()
    {
        if (await woldsHrDbContext.Accounts.AnyAsync())
            return;
        ...
        woldsHrDbContext.Accounts.Add(account);
        await woldsHrDbContext.SaveChangesAsync();
    }

    private async Task<List<Department>?> SeedDepartmentsAsync()
    {
        if (await woldsHrDbContext.Departments.AnyAsync())
            return null;
        ...
        woldsHrDbContext.Departments.AddRange(departments);
        await woldsHrDbContext.SaveChangesAsync();

        return departments;
    }

    private async Task SeedEmployeesAsync(List<Department> departments)
    {
        if (await woldsHrDbContext.Employees.AnyAsync())
            return;

        var employees = GetEmployeeDefaultData(departments).Concat(GetRandomEmployeeDefaultData(departments));
        woldsHrDbContext.Employees.AddRange(employees);
        await woldsHrDbContext.SaveChangesAsync();
    }

    private List<Employee> GetEmployeeDefaultData(List<Department> departments)
    {
    ...
    private List<Employee> GetRandomEmployeeDefaultData(List<Department> departments)
    {
